...
async def help(ctx):
  if ctx.channel.name == "moderator-only":
    ...
    embed.add_field(name="w!list_members", value="Print each member in a separate message *new subcommands*", inline=True)
    embed.add_field(name="w!list_members no_outside",value="List commands command without Outside-Aviation")
    ...
    embed.set_footer(text="Moderator Help command - version 1.2a")
    ...
    embed.set_footer(text="Basic everybody help command - Version 1.2a")
    await ctx.send(embed=embed)
